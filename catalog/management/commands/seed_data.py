# Положите этот файл по пути:
# catalog/management/commands/seed_data.py
#
# Предварительно создайте папки (если их нет):
...
from catalog.models import (
    Role, ProsthesisType, RequestStatus,
    User, UserRole,
    Prosthesis, Request,
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        prostheses = self._create_prostheses(types)
    ...
    def _create_prosthesis_types(self):
        ...
        for name, desc in data:
            ...
            types.append(t)
            self.stdout.write(f'  ✓ Тип: {name}')
        return types

    # ──────────────────────────────────────────
    def _create_statuses(self):
        data = ['Новая', 'В обработке', 'Ожидает документов', 'Одобрена', 'Отклонена', 'Завершена']
        statuses = []
        for name in data:
            s = RequestStatus.objects.create(name=name)
            statuses.append(s)
            self.stdout.write(f'  ✓ Статус: {name}')
        return statuses

    # ──────────────────────────────────────────
    def _create_users(self, roles):
        raw_users = [
            ('Иванов',     'Алексей',   'ivanov@example.com',   '+7 900 111-22-33', [0, 3]),  # Админ + Клиент
            ('Петрова',    'Мария',     'petrova@example.com',  '+7 900 222-33-44', [1, 2]),  # Менеджер + Редактор
            ('Сидоров',    'Дмитрий',   'sidorov@example.com',  '+7 900 333-44-55', [3]),     # Клиент
            ('Козлова',    'Анна',      'kozlova@example.com',  None,               [3]),     # Клиент (нет телефона)
            ('Новиков',    'Игорь',     'novikov@example.com',  '+7 900 555-66-77', [1]),     # Менеджер
            ('Морозова',   'Светлана',  'morozova@example.com', '+7 900 666-77-88', [2]),     # Редактор
            ('Волков',     'Андрей',    'volkov@example.com',   None,               [3]),     # Клиент
            ('Лебедева',   'Ольга',     'lebedeva@example.com', '+7 900 888-99-00', [3]),     # Клиент
        ]
        users = []
        for i, (last, first, email, phone, role_idxs) in enumerate(raw_users):
            ...
            users.append(u)
            self.stdout.write(f'  ✓ Пользователь: {last} {first}')
        return users

    # ──────────────────────────────────────────
    def _create_prostheses(self, types):
        data = [
            ...
            (1, 'Протез стопы Sach',
             ...
             None, True),  # цена не указана
        ]
        prostheses = []
        for type_idx, name, desc, price, is_active in data:
            p = Prosthesis.objects.create(
                prosthesis_type=types[type_idx],
                name=name,
                description=desc,
                price=price,
                image_url=f'https://example.com/images/prosthesis_{len(prostheses)+1}.jpg',
                is_active=is_active,
                created_at=timezone.now() - timedelta(days=random.randint(1, 200)),
            )
            prostheses.append(p)
            self.stdout.write(f'  ✓ Протез: {name[:50]}')
        return prostheses

    # ──────────────────────────────────────────
    def _create_requests(self, users, prostheses, statuses):
        data = [
            # (user_idx, prosthesis_idx, status_idx, type, contact_name, email, message)
        ...
        for i, (u_idx, p_idx, s_idx, rtype, cname, cemail, msg) in enumerate(data):
            ...
            self.stdout.write(f'  ✓ Заявка #{i+1}: {cname}')
    ...
    def _create_blog_posts(self, users):
        data = [
            (1, 'Современные биоэлектрические протезы: как это работает',
             ...
             'Современные системы позволяют выполнять до 24 различных захватов.',
            ...
            (1, 'Экзоскелеты в медицине: настоящее и будущее',
             ...
             'Клинические исследования показывают улучшение нейропластичности при регулярных тренировках.',
            ...
            (5, 'Как выбрать протез: советы специалиста',
             ...
             'Обязательна консультация протезиста и пробный период использования.',
             False, None),  # черновик, не опубликован

            (1, 'Детское протезирование: особенности и подходы',
             'Дети растут быстро, поэтому детские протезы требуют замены каждые 1–2 года. '
             'Важна яркая расцветка и лёгкость конструкции для психологического принятия.',
        ...
        for author_idx, title, content, is_pub, pub_at in data:
            BlogPost.objects.create(
                ...
                is_published=is_pub,
                published_at=pub_at,
                image_url=f'https://example.com/blog/{title[:20].replace(" ","_")}.jpg' if is_pub else None,
            )
            self.stdout.write(f'  ✓ Статья: {title[:50]}')
    ...
    def _create_events(self, users):
        data = [
            ...
            (4, 'Семинар: жизнь с протезом',
             ...
             'Обмен опытом, консультации специалистов.',
            ...
            (4, 'День открытых дверей в центре протезирования',
             ...
             timezone.now() - timedelta(days=10), 'Москва, ул. Медицинская 5'),  # прошедшее
        ...
        for author_idx, title, desc, event_date, location in data:
            Event.objects.create(
                ...
                location=location,
                image_url=f'https://example.com/events/{len(title)}.jpg',
            )
            self.stdout.write(f'  ✓ Мероприятие: {title[:50]}')

    # ──────────────────────────────────────────
    def _create_documents(self, users):
        data = [
            (0, 'Лицензия на медицинскую деятельность №ЛО-77-01-12345',
             'https://example.com/docs/license_medical.pdf', 'license'),

            (0, 'Сертификат ISO 13485:2016 Медицинские изделия',
             'https://example.com/docs/iso_13485.pdf', 'certificate'),

            (4, 'Сертификат соответствия протеза кисти i-Limb',
             'https://example.com/docs/cert_ilimb.pdf', 'certificate'),

            (4, 'Регистрационное удостоверение Росздравнадзора',
             'https://example.com/docs/roszdrav_reg.pdf', 'license'),

            (1, 'Договор с поставщиком Ottobock',
             'https://example.com/docs/contract_ottobock.pdf', 'other'),

            (0, 'Сертификат системы менеджмента качества',
             'https://example.com/docs/smk_cert.pdf', 'certificate'),
        ]
        for uploader_idx, title, file_url, doc_type in data:
